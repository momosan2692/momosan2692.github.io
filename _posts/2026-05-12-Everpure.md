...
下圖整合了電力、記憶體與 Token 產出三個變量，視覺化了 SanDisk 商品主場到 Everpure 企業主場的臨界轉換結構：

![三變量轉折點預測圖](/assets/img/charts/everpure-pivot-chart.svg)

圖表揭示了三條根本不同的相關性曲線，以及它們的商業含義：

**電力（藍線）**：與 Token 產出成完美線性關係（r ≈ 0.95）。每一個 token 都必須過 GPU compute，這是物理定律。這條線是 AI 基礎設施擴張的通用指標，對 SanDisk 或 Everpure 無差別。

**商品 SSD 需求（灰色虛線）**：穩定成長，不受 HBM 臨界點影響，反映儲存容量的總體擴張。這是 SanDisk 的主場——量價齊升的邏輯在臨界點前後都成立，模型權重儲存、訓練數據存放、備份與快取持續產生商品 NAND 需求。

**企業級 SSD 需求（綠線）**：這才是圖表的核心洞察。臨界點之前幾乎是平的（r ≈ 0.30）。一旦 Agentic AI 的平均 context window 使 KV cache 溢出 GPU cluster 的 HBM 總量，這條線開始指數式上升（r ≈ 0.88）——而且這個需求的延遲一致性要求，商品 SSD 架構無法穩定滿足。

**轉折點（Pivot Point）** 是圖表上的紅色實心圓點——KV cache 溢出 HBM、Everpure 進入推論 critical path 的那一刻。這不是市場認知的轉變，是物理條件達到後的必然結果。

Q4 2026（藍色區間）位於轉折點右側，已處於 Everpure 的主場範圍。前提是 Agentic AI 的企業規模採用在這個時間窗口使平均 context 越過 HBM 上限——而這個條件，正在每一個月的 AI 推論成本報告中被悄悄驗證。
