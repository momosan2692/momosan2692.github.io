...
### Node 2 — Diagnostic Reasoning Node (PHI node)

**Runs:** vMLX serving a reasoning-capable LLM; Agent 2 (Diagnostic Reasoning) process  
**PHI status:** PHI lives here — receives clinical context from sub-task B (with PHI) to perform differential diagnosis reasoning  
...
**Model target:** `mlx-community/Qwen3-8B-4bit` with `--reasoning-parser auto` (strong CoT capability on Apple Silicon)  
...
STEP 5  MERGE, SANITIZE & SYNTHESIZE
        ...
        Route to cloud model (Agent 3 / Anthropic) for final synthesis generation.
...
```json
{
  ...
  "medical_aggregator": {
    "synthesis_agent": "cloud",
    "synthesis_model": "claude-sonnet-4-6",
    "synthesis_api_key_env": "ANTHROPIC_API_KEY",
...
### M2 — Clinical data fixtures (week 1–2)
- [ ] Prepare FHIR R4 format clinical fixture dataset (synthetic patients — no real PHI in dev)
- [ ] Build FAISS vector index on Node 1 from fixture corpus using bge-m3 embeddings
- [ ] Implement Agent 1 stub: FHIR fixture retrieval + vMLX synthesis; verify round-trip via tunnel
...
- [ ] Implement `medical_synthesize` — de-identified synthesis input + disclaimer append
...
|---|----------|---------|---------|
| 1 | **Node 1 model** | BioMistral-7B-4bit, Llama3-OpenBioLLM-8B-4bit, MedAlpaca-7B-4bit | BioMistral-7B-4bit (best availability on HuggingFace MLX) |
| 2 | **Node 2 model** | Qwen3-8B-4bit, Llama3-8B-4bit, Mistral-7B-4bit | Qwen3-8B-4bit (strong CoT on Apple Silicon) |
| 3 | **Clinical vector store** | FAISS (local, no server), pgvector, Qdrant | FAISS for P1 |
| 4 | **Embedding model** | bge-m3, text-embedding-3-small (cloud), NV-Embed-v2 | bge-m3 (on-premise, no PHI leaves node) |
| 5 | **Literature APIs** | PubMed only, PubMed + Cochrane + WHO, UpToDate (paid) | PubMed + WHO for P1 (free tier) |
| 6 | **Drug/Rx APIs** | FDA Drug API (free), DrugBank (licensed), RxNorm | FDA + RxNorm for P1 (free) |
| 7 | **De-id implementation** | Presidio (rule-based), GLiNER (NER model), spaCy NER | Presidio for P1 (no ML model needed) |
| 8 | **Synthesis step** | Always cloud (Anthropic), Node 2 vMLX, dynamic | Cloud (Anthropic claude-sonnet-4-6) for P1 |
| 9 | **Audit log backup** | Manual rsync, Time Machine, automated offsite | Manual rsync P1; automated P2 |
